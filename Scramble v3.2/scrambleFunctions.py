...
## Get the correct level of difficulty
def game_word(d, words):

    p = int(d)

    # (very easy [3], easy [5], moderate [7], hard [9], very hard [])
    if p == 1:
        q = 3
    elif p == 2:
        q = 5
    ...
    wrd = get_valid_word(words)

    while ( len(wrd) < (q-2) or len(wrd) > q ):
...
def scrumble(wrd):
    ...
    while len(word) > 0:
        letter = str( random.choice(word) )
        letters.append( letter )
        word.remove(letter)

    return ''.join(letters)                 # join list characters to form string
